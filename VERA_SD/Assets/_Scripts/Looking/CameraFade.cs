...
public class CameraFade : MonoBehaviour
{
    ...
    public IEnumerator RunFade(Camera playerCam, GameObject target)
    {
        //Debug.Log(Time.time);
        Fade();
        yield return new WaitUntil(() => alpha == 1f);
        playerCam.transform.parent.LookAt(target.transform);
        //Debug.Log(Time.time);
        yield return new WaitForSeconds(speedScale);
        Fade();
    }
    private void Fade()
    {
